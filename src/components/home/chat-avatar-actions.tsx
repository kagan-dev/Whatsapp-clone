...
const ChatAvatarActions = ({ me, message }: ChatAvatarActionsProps) => {
  ...
  const isMember = selectedConversation?.partipications.includes(
    message.sender._id
  );
  ...
  const handleKickUser = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      setselectedConversation({
        ...
        partipications: selectedConversation.partipications.filter(
  ...
  const handleCreateConversation = async () => {
    ...
    try {
      const conversationId = await createConversation({
        isGroup: false,
        partipications: [me._id, message.sender._id],
      });
